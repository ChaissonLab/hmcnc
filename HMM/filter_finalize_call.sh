...
## usage:
## ./filter_finalize_call.sh <calls.bed>  HMM/annotation/region_EXCLUDE.no_alts.bed  HMM/annotation/region_EXCLUDE.no_alts.bed   


call=$1

grep PASS $call |awk '$4>2' | cut -f 1-8 | intersectBed -v -a stdin -b $2 |\
intersectBed -a stdin -b $3 -sorted  -loj| \
...
grep PASS $call | awk '$4>2' |cut -f 1-8 |mergeBed| intersectBed -v -a stdin -b $2 | \
intersectBed -a stdin -b $3 -sorted  -loj| \
...
grep PASS $call | awk '$4<2' |cut -f 1-8|intersectBed -v -a stdin -b $2 > del.filtered_final.$call
